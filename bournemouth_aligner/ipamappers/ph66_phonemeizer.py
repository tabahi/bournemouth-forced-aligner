...
class Phonemizer:
    ...
    def print_mapping_summary(self):
        ...
        - `word_counts` are the total number of words processed (including those that may not have valid phonemes).
        - `phn_counts` are the total number of phonemes counted (after mapping and filtering).
        - `phoneme_vocab_counts` shows the count of each original phoneme from espeak before mapping.
        - `phoneme_vocab_mapped_counts` shows the count of each mapped phoneme that was actually included in the output.
        - `unmapped_phonemes` lists phonemes that were generated by espeak but not found in the mapping, which are treated as noise.
        - `sil_count` and `noise_count` track how many silence and noise phonemes were included in the output, which can be useful for analyzing the quality of the phonemization and the amount of noise.
    ...
    def phonemize_sentence(self, sentence):
        ...
        for word_idx, (word, phonemized) in enumerate(zip(words, phonemized_words)):
            ...
            if word == "<SIL>":
                ...
                self.phoneme_vocab_mapped_counts[SIL_PHN] += 1
                self.sil_count += 1
                continue
            ...
            for eph in word_phonemes:
                ...
                for phi in range(len(ph_comp)):
                    ...
                    if self.remove_noise_phonemes and ph == noise_PHN:
                        self.noise_count += 1
                        self.phoneme_vocab_mapped_counts[noise_PHN] += 1
                    ...
                    self.phoneme_vocab_mapped_counts[ph] += 1
